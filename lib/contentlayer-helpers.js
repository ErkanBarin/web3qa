...
function isPublished(item) {
  ...
  if (typeof item.isValid === 'boolean' && !item.isValid) return false;
  return true;
}
...
export function getAllBySection(section) {
  ...
  const all = [
    ...safeArray(contentlayer.allArticles || []),
    ...safeArray(contentlayer.allGlossaryTerms || [])
  ];
...
export function glossaryIndex() {
  const terms = safeArray(contentlayer.allGlossaryTerms || [])
    .filter(t => isPublished(t))
    .map(t => ({ term: t.term || t.title, slug: t.slug }));
