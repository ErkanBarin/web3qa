...
async function analyzeContracts() {
  ...
  try {
    ...
    if (results.success && results.results.detectors) {
      const issues = results.results.detectors;
      console.log(`✅ Slither analysis complete. Issues found: ${issues.length}`);
      if (issues.length > 0) {
        issues.forEach(issue => console.log(`- ${issue.check} (${issue.impact}): ${issue.description}`));
      ...
      const results = JSON.parse(errorOutput);
       if (results.success && results.results.detectors) {
        ...
        console.log(`✅ Slither analysis complete. Issues found: ${issues.length}`);
         ...
         console.error('Failed to parse Slither output from error stream:', results.error);
         process.exit(1);
       }
    } catch (parseError) {
      console.error('Failed to execute or parse Slither:', error.stderr);
      process.exit(1);
    }
  }
}
