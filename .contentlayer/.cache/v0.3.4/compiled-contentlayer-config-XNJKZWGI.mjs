...
var Article = defineDocumentType(() => ({
  ...
  computedFields: {
    ...
    normalizedTags: {
      type: "list",
      resolve: (doc) => (doc.tags || []).map((t) => slugify(t)).filter(Boolean).filter((v, i, a) => a.indexOf(v) === i).sort()
    },
    isValid: {
      type: "boolean",
      resolve: (doc) => {
        ...
        if (doc.chain && !CHAINS.includes(doc.chain.toLowerCase()))
          return false;
        if (doc.tags && doc.tags.some((t) => !TAGS.includes(slugify(t))))
          return false;
        return true;
      }
    },
...
//# sourceMappingURL=compiled-contentlayer-config-XNJKZWGI.mjs.map
