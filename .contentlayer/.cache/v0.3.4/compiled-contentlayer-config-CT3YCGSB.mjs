    ...
    normalizedTags: {
      ...
      resolve: (doc) => {
        ...
        if (!Array.isArray(tags))
          return [];
        return tags.map((t) => slugify(String(t))).filter(Boolean).filter((v, i, a) => a.indexOf(v) === i).sort();
      }
    },
    isValid: {
      type: "boolean",
      resolve: (doc) => {
        ...
        if (doc.chain && !CHAINS.includes(doc.chain.toLowerCase()))
          return false;
        if (Array.isArray(doc.tags) && doc.tags.some((t) => !TAGS.includes(slugify(t))))
          return false;
        return true;
      }
    },
...
//# sourceMappingURL=compiled-contentlayer-config-CT3YCGSB.mjs.map
