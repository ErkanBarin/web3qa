...
var TAGS = ["testing", "security", "tooling", "guides", "glossary", "architecture", "best-practices", "performance"];
var SECTIONS = ["guides", "tools", "concepts", "patterns", "tutorials", "glossary", "references", "faq"];
...
var Article = defineDocumentType(() => ({
  ...
  computedFields: {
    ...
    normalizedTags: {
      ...
      resolve: (doc) => {
        const tags = doc.tags;
        console.log("Processing tags for", doc.title, ":", tags, "type:", typeof tags, "isArray:", Array.isArray(tags));
        if (!Array.isArray(tags))
          return [];
        const result = tags.map((t) => String(t).toLowerCase().trim()).filter((t) => TAGS.includes(t)).filter((v, i, a) => a.indexOf(v) === i).sort();
    ...
    isValid: {
      ...
      resolve: (doc) => {
        ...
        if (doc.chain && !CHAINS.includes(doc.chain.toLowerCase()))
          return false;
        if (Array.isArray(doc.tags) && doc.tags.some((t) => !TAGS.includes(String(t).toLowerCase().trim())))
          return false;
        return true;
      }
    },
...
//# sourceMappingURL=compiled-contentlayer-config-JIYO2STZ.mjs.map
